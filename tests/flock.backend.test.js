...
test('api_saveInteraction updates interaction + people and auto-closes open followups', () => {
  ...
  assert.ok(people[5] instanceof Date, 'LastAttempt should be date');
  assert.ok(people[6] instanceof Date, 'LastSuccessfulContact should be date');
  assert.equal(people[8], 'Completed');
  assert.ok(people[7] instanceof Date, 'NextDueDate should be date');

  const followup = h.spreadsheet.getSheetByName('FOLLOWUPS').rows[1];
  assert.equal(followup[5], 'Done');
  assert.ok(followup[7] instanceof Date, 'CompletedAt should be date');
  assert.match(String(followup[8]), /Auto-closed/i);

  assert.equal(h.scriptCache.get('duePeople'), null, 'cache should be busted on write');
});
